...
This project uses Prettier and ESLint for automatic code formatting and linting. The setup ensures consistent code style across the project.

## Features

- **Automatic formatting on save**: Files are automatically formatted when saved in VS Code
- **Pre-commit hooks**: Code is automatically formatted and linted before each commit
- **Tailwind CSS optimization**: Tailwind CSS classes are automatically sorted in an optimal way
...
The project includes VS Code settings that enable format on save. Make sure you have the Prettier extension installed in VS Code.

### Pre-commit Hooks

The project uses Husky to run lint-staged before each commit. This ensures that all committed code is properly formatted and linted.
